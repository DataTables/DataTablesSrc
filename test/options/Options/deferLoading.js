describe( "deferLoading option", function() {
	...
	describe('Check Default', function() {
		...
		it('deferLoading disabled by default', function() {
			$('#example').dataTable();
			expect(DataTable.defaults.deferLoading).toBe(null);
		});
	});
	...
	describe('Functional tests', function() {
		...
		it('Single number (rows)', function(done) {
			let table = $('#example').DataTable({
				...
				initComplete: function(setting, json) {
					expect($('div.dt-info').text()).toBe('Showing 1 to 10 of 33 entries');
					done();
				}
			});
		});
		...
		it('Two numbers (rows and total length)', function(done) {
			let table = $('#example').DataTable({
				...
				initComplete: function(setting, json) {
					expect($('div.dt-info').text()).toBe('Showing 1 to 10 of 33 entries (filtered from 22 total entries)');
					done();
				}
			});
		});
