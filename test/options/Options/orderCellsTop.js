describe('orderCellsTop option (legacy)', function() {
	dt.libs({
	...
	describe('Check the defaults', function() {
		...
		dt.html('basic');
		it('Can be turned off- Using 2 rows in header', function() {
			$('#example thead').append(
				'<tr><th>Name1</th><th>Position1</th><th>Office1</th><th>Age1</th><th>Start date1</th><th>Salary1</th></tr>'
			);
			let table = $('#example').DataTable({
	...
	describe('Check when using 3 rows in header', function() {
		dt.html('basic');
		it('Not used by default- Using 3 rows in header', function() {
			$('#example thead').append(
				'<tr><th>Name1</th><th>Position1</th><th>Office1</th><th>Age1</th><th>Start date1</th><th>Salary1</th></tr>'
			);
			$('#example thead').append(
		...
		dt.html('basic');
		it('Can be turned off- Using 3 rows in header', function() {
			$('#example thead').append(
				'<tr><th>Name1</th><th>Position1</th><th>Office1</th><th>Age1</th><th>Start date1</th><th>Salary1</th></tr>'
			);
			$('#example thead').append(
	...
	describe('Ignored when using ColSpan- 2 rows', function() {
		dt.html('basic');
		it('Not used by default when using ColSpan- 2 rows', function() {
		...
		it('Enabled when using ColSpan- 2 rows', function() {
			...
			expect($('#example thead tr:eq(0) th:eq(0)').hasClass('dt-ordering-asc')).toBe(false);
			expect($('#example thead tr:eq(1) th:eq(0)').hasClass('dt-ordering-asc')).toBe(false);
			expect($(table.column(0).header()).text()).toBe('HR Information');
		});

		dt.html('basic');
		it('Disabled when using ColSpan- 2 rows', function() {
			$('#example thead').prepend('<tr><th colspan="3">HR Information</th><th colspan="3">Contact</th></tr>');
			let table = $('#example').DataTable({
				orderCellsTop: false
			});
	...
	describe('When using RowSpan', function() {
		dt.html('basic');
		it('Not used by default when using RowSpan (first column, 2 rows)', function() {
		...
		dt.html('basic');
		it('Note used by default when using RowSpan (second column, 2 rows)', function() {
			$('#example thead').prepend(
