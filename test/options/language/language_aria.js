describe('language.aria option ', function () {
	...
	describe('Check the defaults', function () {
		...
		it('Can we set pagination aria controls different from normal paginate', function () {
			...
			expect(
				$('.dt-paging-button[data-dt-idx=0]').attr('aria-label')
			).toBeUndefined();
			expect(
				$('.dt-paging-button[data-dt-idx=1]').attr('aria-label')
			).toBeUndefined();
		});
		...
		it('Pagination number aria label', function () {
			...
			expect($('.dt-paging-button.first').attr('aria-label')).toBe('First');
			expect($('.dt-paging-button.last').attr('aria-label')).toBe('Last');
			expect($('.dt-paging-button[data-dt-idx=0]').attr('aria-label')).toBe(
				'Jump to page: 1'
			);
			expect($('.dt-paging-button[data-dt-idx=1]').attr('aria-label')).toBe(
				'Jump to page: 2'
			);
		});
