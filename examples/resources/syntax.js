...
hljs.addPlugin({
  'before:highlightElement': ({ el }) => {
    // Get the raw text content from the element
    const rawCode = el.textContent;

    // Split into lines and remove empty trailing/leading lines
    ...
    // If we found a common indent, strip it from every line
    if (minIndent !== Infinity && minIndent > 0) {
      el.textContent = lines
        .map(line => line.slice(minIndent))
        .join('\n')
        .trim(); // Optional: trim to remove extra top/bottom padding
    }
  }
});
